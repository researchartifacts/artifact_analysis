...
_EXTRA_COLORS = [
    ...
    '#8B008B', '#FF4500', '#2F4F4F', '#DAA520', '#00FA9A',
    '#C71585', '#1E90FF', '#B22222', '#556B2F',
]
...
def create_category_timeline_chart(by_conference, category, output_path):
    ...
    for i, label in enumerate(labels):
        ...
        ax.plot(years, vals, label=label, color=_color_for(label, i),
...
def create_coverage_table(by_conference, output_path):
    ...
    header_w = max(2.5, max(len(l) for l in labels) * 0.12)
    ...
    for i, label in enumerate(labels):
        ...
        ax.text(header_w - 0.15, row_y + cell_h / 2, label,
        ...
        for j, year in enumerate(years):
            ...
            if count is not None and count > 0:
                ...
                color = '#f2f2f2'
                text = '—'
            rect = plt.Rectangle((x, row_y), cell_w, cell_h,
                                 facecolor=color, edgecolor='#cccccc', linewidth=0.5)
            ax.add_patch(rect)
            ax.text(x + cell_w / 2, row_y + cell_h / 2, text,
                    ha='center', va='center', fontsize=7,
                    color='#333333' if count else '#999999')

    ax.set_title('Conference Coverage (artifact count per year)', fontsize=11,
...
def generate_all_charts(data_dir):
    ...
    create_category_timeline_chart(
        by_conference, 'systems',
        os.path.join(charts_dir, 'systems_artifacts.svg')
    )
    create_category_timeline_chart(
        by_conference, 'security',
        os.path.join(charts_dir, 'security_artifacts.svg')
    )
    
    # Total artifacts (stacked systems + security)
    create_total_artifacts_chart(
        by_year,
        os.path.join(charts_dir, 'total_artifacts.svg')
    )
    
    # Badge chart (combines distribution + trends into one line chart)
    create_badge_distribution_chart(
        all_artifacts,
        os.path.join(charts_dir, 'badge_distribution.svg')
    )
    
    # Coverage table
    create_coverage_table(
        by_conference,
        os.path.join(charts_dir, 'coverage_table.svg')
    )
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate visualizations for research artifacts website'
    )
    parser.add_argument(
        '--data_dir',
        type=str,
        required=True,
        help='Directory containing the generated data files'
    )
