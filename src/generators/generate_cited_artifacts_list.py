...
def generate(data_dir: str) -> None:
    ...
    cited_by_author = defaultdict(lambda: {
        "display_name": "",
        "affiliation": "",
        "display_affiliation": "",
        "cited_artifacts": [],
    ...
    cited_by_institution = defaultdict(lambda: {
        "display_name": "",
        "cited_artifacts": [],
        "total_citations": 0,
        "member_count": 0,
        ...
        if paper:
            ...
            for author_name in authors:
                ...
                if affiliation:
                    ...
                    if not any(a["title"] == artifact_entry["title"] 
    ...
    with open(out_author_cited, "w") as f:
        ...
        sort_dict = {k: out_dict[k] for k in sorted(out_dict.keys(), 
                    key=lambda x: out_dict[x]["total_citations"], reverse=True)}
    ...
    with open(out_cited_list, "w") as f:
        ...
        sorted_list = sorted(cited_artifacts_list, 
                           key=lambda x: x["cited_by_count"], reverse=True)
