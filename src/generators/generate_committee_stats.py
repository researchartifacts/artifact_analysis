...
def _compute_recurring_members(all_results, conf_to_area, classified):
    ...
    for rec in all_members:
        entry = {
            ...
            'conferences': sorted([[c, y, rec['roles_by_conf'].get(cy, 'member')] for cy in rec['conference_years'] for c, y in [_extract_conf_year(cy)]], key=lambda x: (x[0], x[1] or 0)),
            'area': rec['area'],
    ...
    for rec in all_members:
        ...
        entry = {
            ...
            'conferences': sorted([[c, y, rec['roles_by_conf'].get(cy, 'member')] for cy in rec['sys_conference_years'] for c, y in [_extract_conf_year(cy)]], key=lambda x: (x[0], x[1] or 0)),
            'area': rec['area'],
    ...
    for rec in all_members:
        ...
        entry = {
            ...
            'conferences': sorted([[c, y, rec['roles_by_conf'].get(cy, 'member')] for cy in rec['sec_conference_years'] for c, y in [_extract_conf_year(cy)]], key=lambda x: (x[0], x[1] or 0)),
            'area': rec['area'],
